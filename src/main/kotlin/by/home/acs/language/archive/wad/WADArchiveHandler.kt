...
import by.home.acs.language.archive.util.getAndUse
...
open class WADArchiveHandler(path: String) :
    BaseArchiveHandler<WADArchiveHandler.WADArchiveHolder>(path) {
    class WADArchiveHolder(file: File) : Closeable {
        ...
        fun <R> useStream(block: (WADArchiveHolder) -> R): R {
            return stream.use {
                block(this)
            }
        }
    ...
    override val accessorCache: FileAccessorCache<BaseArchiveHandler<WADArchiveHolder>, WADArchiveHolder> =
        CACHE

    companion object {
        private val CACHE = createCache<WADArchiveHolder>(
            onCreate = {
                WADArchiveHolder(it.file.canonicalFile)
            },
            onDispose = {
                it.close()
            },
    ...
    private fun getItemForPath(holder: WADArchiveHolder, relativePath: String): ISimpleInArchiveItem? {
    ...
    override fun getInputStream(relativePath: String): InputStream {
        return getFileHandle().getAndUse { holder ->
            ...
            WADInputStream(holder, item)
        }
    }

    fun getInputStreamForFile(file: VirtualFile): WADInputStream {
        return getFileHandle().getAndUse { holder ->
            val item = getItemForPath(holder, file.path.split(FileStructureUtil.FS_SEPARATOR).last())
    ...
    private val ISimpleInArchiveItem.ideaPath
        get() = FileStructureUtil.convertPathToIdea(path)


    class WADInputStream(val holder: WADArchiveHolder, private val item: ISimpleInArchiveItem) :
        InputStream() {
